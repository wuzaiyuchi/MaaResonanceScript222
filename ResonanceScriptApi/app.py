...
async def maaInit():
    ...
    if not maa_inst.inited:
        ...
        exit()
    test1 = await maa_inst.run_task("幅度")
    test2 = await maa_inst.run_recognition("幅度")
    print(test2)
    # return test2
...
@app.route('/runTask2', methods=["POST"])
def runTask2():
    q_data = request.get_data()  # 接收json数据
    data = json.loads(q_data)  # json数据解析
    taskId = data["taskId"]
    ...
    # resp= asyncio.run(run_task(taskId,target))
    resp = asyncio.run(run_task2(taskId, target))
    return str(resp)
...
async def run_task2(taskId, param):
    return await maafw.run_task(taskId, param)
